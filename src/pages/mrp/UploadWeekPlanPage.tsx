...
const STORAGE_KEY = 'mrp:weekPlan:uploads:v1';
const LAST_PARSED_KEY = 'mrp:weekPlan:lastParsed:v1';
...
function scoreWeekPlanSheet(sheetName: string, aoa: unknown[][]) {
...
export default function UploadWeekPlanPage() {
  ...
  useEffect(() => {
    document.title = 'MRP - Upload Week Plan';
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const files = await filesApi.list({ type: 'week_plan', limit: 25, offset: 0 });
        if (cancelled) return;
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      try {
        const uploaded = await filesApi.upload(selectedFile, 'week_plan');
      ...
      await Swal.fire({
        ...
        text: backendFileId
          ? 'Week Plan file uploaded successfully'
          : 'Week Plan file saved',
      });
  ...
  const handleCompareWithLast = async () => {
    ...
    const findKeyIndex = (headers: unknown[]) => {
      const idx = headers.findIndex((h) => {
        const s = normalizeHeaderValue(h);
        return s === 'week no' || s.includes('week no') || s === 'week' || s.includes('week');
      });
      return idx >= 0 ? idx : 0;
    };
    ...
    const html = `
      <div style="text-align:left">
        ...
        <div style="margin-top:12px; display:grid; grid-template-columns: 1fr 1fr 1fr; gap:10px">
          <div style="border:1px solid #e2e8f0; border-radius:10px; padding:10px">
            <div style="color:#64748b; font-size:12px">Added weeks</div>
          ...
          <div style="border:1px solid #e2e8f0; border-radius:10px; padding:10px">
            <div style="color:#64748b; font-size:12px">Removed weeks</div>
          ...
          <div style="border:1px solid #e2e8f0; border-radius:10px; padding:10px">
            <div style="color:#64748b; font-size:12px">Modified weeks</div>
  ...
  const handleDeleteRecord = async (id: string) => {
    ...
    if (rec?.backendFileId) {
      try {
        await filesApi.remove(rec.backendFileId, 'week_plan');
      } catch {
        await Swal.fire({
          icon: 'warning',
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl bg-gradient-to-r from-slate-900 to-indigo-700 p-6 text-white shadow-sm">
        <div className="text-2xl font-semibold">Upload Week Plan</div>
        <div className="mt-1 text-sm text-white/80">Upload and manage Week Plan excel files.</div>
      </div>
