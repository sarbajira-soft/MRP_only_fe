...
const STORAGE_KEY = 'mrp:bom:uploads:v1';
const LAST_PARSED_KEY = 'mrp:bom:lastParsed:v1';
...
export default function UploadBomPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const files = await filesApi.list({ type: 'bom', limit: 25, offset: 0 });
        if (cancelled) return;
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      try {
        const uploaded = await filesApi.upload(selectedFile, 'bom');
      ...
      await Swal.fire({
        ...
        text: backendFileId
          ? 'BOM file uploaded successfully'
          : 'BOM file saved',
      });
  ...
  const handleCompareWithLast = async () => {
    ...
    if (!last) {
      ...
      if (latestBackend?.backendFileId) {
        try {
          const buf = await filesApi.downloadArrayBuffer(latestBackend.backendFileId, 'bom');
          ...
          await Swal.fire({
            ...
            text: 'No previous BOM upload found to compare',
          });
          return;
        }
      } else {
        await Swal.fire({ icon: 'info', title: 'No Previous Upload', text: 'No previous BOM upload found to compare' });
        return;
      }
    }
    ...
    const normalizeHeader = (h: unknown) => String(h ?? '').trim().toLowerCase().replaceAll(/\s+/g, ' ');
    ...
    const findColIndex = (headers: unknown[], candidates: string[]) => {
    ...
    const keyColumnNames = ['Derived Material', 'Child Part', 'Stage'];
    const keyColumnIndices = [
      findColIndex(newHeaders, ['Derived Material', 'DerivedMaterial']),
      findColIndex(newHeaders, ['Child Part', 'ChildPart', 'Child']),
    ...
    newRows.forEach((row) => {
      newRowsMap.set(createRowKey(row), row);
    });

    const lastRowsMap = new Map<string, unknown[]>();
    lastRows.forEach((row) => {
      lastRowsMap.set(createRowKey(row), row);
    });

    const addedRows = newRows.filter((row) => !lastRowsMap.has(createRowKey(row)));
    const removedRows = lastRows.filter((row) => !newRowsMap.has(createRowKey(row)));
    ...
    const headerCells = newHeaders.map((h) => `<th style="position:sticky; top:0; background:#0f172a; color:#fff; padding:8px; border-bottom:1px solid #334155">${escape(h)}</th>`).join('');
    ...
    const html = `
      <div style="text-align:left">
        ...
        <div style="margin-bottom:10px; color:#0f172a; font-size:13px">
          Comparing: <strong>${escape(oldFileName || 'Previous')}</strong> → <strong>${escape(newFileName || parsed.sheetName || 'Selected')}</strong>
        </div>
  ...
  const handleDeleteRecord = async (id: string) => {
    ...
    if (rec?.backendFileId) {
      try {
        await filesApi.remove(rec.backendFileId, 'bom');
      } catch {
        await Swal.fire({
          icon: 'warning',
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl bg-gradient-to-r from-slate-900 to-indigo-700 p-6 text-white shadow-sm">
        <div className="text-2xl font-semibold">Upload BOM</div>
        <div className="mt-1 text-sm text-white/80">Upload and manage BOM excel files.</div>
      </div>
