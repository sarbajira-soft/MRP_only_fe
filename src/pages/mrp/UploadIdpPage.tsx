...
const STORAGE_KEY = 'mrp:idp:uploads:v1';
const LAST_PARSED_KEY = 'mrp:idp:lastParsed:v1';
...
function scoreIdpSheet(sheetName: string, aoa: unknown[][]) {
...
async function parseExcel(file: File): Promise<ParsedExcel> {
  ...
  workbook.SheetNames.forEach((sn) => {
    ...
    const score = scoreIdpSheet(sn, aoa);
    if (score > bestScore) {
      bestScore = score;
      bestSheetName = sn;
      bestJsonData = aoa;
...
export default function UploadIdpPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const files = await filesApi.list({ type: 'idp', limit: 25, offset: 0 });
        if (cancelled) return;
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      try {
        const uploaded = await filesApi.upload(selectedFile, 'idp');
  ...
  const handleCompareWithLast = async () => {
    ...
    if (!last) {
      ...
      if (latestBackend?.backendFileId) {
        try {
          const buf = await filesApi.downloadArrayBuffer(latestBackend.backendFileId, 'idp');
          ...
          await Swal.fire({ icon: 'info', title: 'No Previous Upload', text: 'No previous IDP upload found to compare' });
          return;
        }
      } else {
        await Swal.fire({ icon: 'info', title: 'No Previous Upload', text: 'No previous IDP upload found to compare' });
  ...
  const handleDeleteRecord = async (id: string) => {
    ...
    if (rec?.backendFileId) {
      try {
        await filesApi.remove(rec.backendFileId, 'idp');
      } catch {
        await Swal.fire({
          icon: 'warning',
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl bg-gradient-to-r from-slate-900 to-indigo-700 p-6 text-white shadow-sm">
        <div className="text-2xl font-semibold">Upload IDP</div>
        <div className="mt-1 text-sm text-white/80">Upload and manage IDP excel files.</div>
      </div>
