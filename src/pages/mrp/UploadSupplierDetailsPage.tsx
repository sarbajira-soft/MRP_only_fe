...
const STORAGE_KEY = 'mrp:supplierDetails:uploads:v1';
const LAST_PARSED_KEY = 'mrp:supplierDetails:lastParsed:v1';
...
function scoreSupplierDetailsSheet(sheetName: string, aoa: unknown[][]) {
...
async function parseExcel(file: File): Promise<ParsedExcel> {
  ...
  workbook.SheetNames.forEach((sn) => {
    ...
    const score = scoreSupplierDetailsSheet(sn, aoa);
    if (score > bestScore) {
      bestScore = score;
      bestSheetName = sn;
      bestJsonData = aoa;
...
export default function UploadSupplierDetailsPage() {
  ...
  useEffect(() => {
    document.title = 'MRP - Upload Supplier Details';
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const files = await filesApi.list({ type: 'supplier_details', limit: 25, offset: 0 });
        if (cancelled) return;
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      try {
        const uploaded = await filesApi.upload(selectedFile, 'supplier_details');
      ...
      await Swal.fire({
        ...
        text: backendFileId
          ? 'Supplier Details file uploaded successfully'
          : 'Supplier Details file saved',
      });
  ...
  const showPreview = async () => {
            ...
            <thead>
              <tr>
                ${headers
                  .map(
                    (h) =>
                      `<th style="position:sticky; top:0; background:#0f172a; color:#fff; padding:8px; border-bottom:1px solid #334155">${escape(h)}</th>`,
                  )
                  .join('')}
              </tr>
            </thead>
    ...
    await Swal.fire({
      title: 'Supplier Details Preview',
      html: tableHtml,
      width: '95%',
  ...
  const handleCompareWithLast = async () => {
    ...
    if (!last) {
      ...
      if (latestBackend?.backendFileId) {
        try {
          const buf = await filesApi.downloadArrayBuffer(latestBackend.backendFileId, 'supplier_details');
          ...
          await Swal.fire({
            ...
            text: 'No previous Supplier Details upload found to compare',
          });
          return;
        }
      } else {
    ...
    const findKeyIndex = (headers: unknown[], q: string, fallback: number) => {
      const idx = headers.findIndex((h) => normalizeHeaderValue(h).includes(q));
      return idx >= 0 ? idx : fallback;
    };

    const materialIdx = findKeyIndex(newHeaders, 'material', 0);
    const vendorIdx = findKeyIndex(newHeaders, 'vendor', Math.min(1, Math.max(0, newHeaders.length - 1)));
    const keyColumnNames = ['Material', 'Vendor'];

    const toKey = (r: unknown[]) => `${String(r[materialIdx] ?? '').trim()}|${String(r[vendorIdx] ?? '').trim()}`;
    ...
    const addedRows = newRows.filter((row) => {
      const k = toKey(row);
      return k && k !== '|' && !lastRowsMap.has(k);
    });

    const removedRows = lastRows.filter((row) => {
      const k = toKey(row);
      return k && k !== '|' && !newRowsMap.has(k);
    });
  ...
  const handleDeleteRecord = async (id: string) => {
    ...
    if (rec?.backendFileId) {
      try {
        await filesApi.remove(rec.backendFileId, 'supplier_details');
      } catch {
        await Swal.fire({
          icon: 'warning',
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl bg-gradient-to-r from-slate-900 to-indigo-700 p-6 text-white shadow-sm">
        <div className="text-2xl font-semibold">Upload Supplier Details</div>
        <div className="mt-1 text-sm text-white/80">Upload and manage Supplier Details excel files.</div>
      </div>
              ...
              {activeUploads.length === 0 ? (
                ...
                activeUploads.map((u, idx) => (
                  <tr key={u.id} className="text-sm">
                    ...
                    <td className="border-b border-slate-100 px-3 py-2 font-mono text-xs text-slate-700">{u.id}</td>
